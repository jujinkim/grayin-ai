...
Each connected Location manual scan reads only Android's current last-known observation. Separately, the user may explicitly start the default-OFF movement-observation mode from the visible app. That mode displays a persistent foreground-service notification with an immediate Stop action, requests no background-location permission, and does not auto-start from WorkManager, boot, or an invisible app state. While active, it accepts only bounded network/GPS observations at a 15-minute / 250-metre threshold; stale, future-dated, excessively inaccurate, and over-frequent fixes are discarded before processing. Grayin derives a stable cluster from the 0.001-degree rounded coordinate and accumulates new source observations in SQLCipher; it neither reads a platform location-history archive nor stores the original exact coordinate. Movement observation never uses online enrichment. With enrichment consent, manual reverse geocoding and weather are attempted independently. A failure leaves coordinate-only or place-only local evidence intact. Optional place labels are normalized and bounded, and stored weather signals contain only closed numeric WMO/temperature/precipitation fields rather than provider prose or errors.
