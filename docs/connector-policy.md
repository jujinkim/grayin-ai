...
The store applies the same schema-v8 closed-record validator to live scans and encrypted imports. Location, Photos, Calendar, Notifications, and App Usage must use their current canonical source/event/citation shapes, fixed provider/category vocabularies, bounded aliases, and allowed aggregate fields; claiming schema v8 cannot bypass those checks with a legacy open record. The current App Usage connector produces canonical per-session events, not `AppUsageSummary` rows, so schema-v8 scans and imports require that aggregate section to be empty until a canonical aggregate producer is implemented.
