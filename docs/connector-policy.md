...
Location supports explicit runtime location permission plus app-level connection before indexing. It reads last-known provider location transiently, emits rounded derived place-visit events/citations/source references, and does not store raw location provider dumps. Android's provider name is reduced to `gps`, `network`, `passive`, `fused`, or `other` before it enters identity, pointer, keyword, or label fields. A separate default-OFF movement-observation switch may be enabled only from the visible app. It starts a `location` foreground service with a persistent notification and immediate Stop action; no Android background-location permission is requested. The service requests network/GPS updates at a 15-minute / 250-metre threshold, discards stale, future, over-frequent, and accuracy-over-5-km samples before connector extraction, and sends each accepted fix through the same rounded connector scan path without online enrichment. Its in-memory writer is serialized with Location revoke/delete, so a completed delete or revoke cannot be followed by a late service commit. A separate default-OFF enrichment switch permits manual Location scans to call typed reverse-geocode and weather methods. Reverse geocoding receives only the transient 0.001-degree coordinate; the weather provider projection rounds it again to 0.01 degrees and transmits one UTC date. The two lookups fail independently. Unavailable/invalid/provider failures discard only that enrichment and always preserve local coordinate indexing; coroutine cancellation remains cancellation. Stored weather output is built only from closed WMO code, temperature, and precipitation numbers and never copies provider text, response bodies, URLs, or errors. Revocation disables both Location and movement-observation consent.
...
- Location remains outside WorkManager. Manual scans are foreground-only; separately, a user-started visible `location` foreground service may collect bounded observations while its persistent notification remains active. Grayin does not request background-location permission.
...
Automatic indexing plans only background-scannable connectors. The user-started Location foreground service is not automatic indexing and never starts from WorkManager, boot, or an invisible app state. Foreground-only connectors remain available through explicit user actions, and event-driven connectors do not show a misleading manual historical-index action.
