...
Artifact supply-chain and interrupted-install risks are reduced with an APK-authenticated closed catalog, immutable HTTPS paths, exact byte counts, pinned SHA-256 digests, redirect/header rejection, flushed same-filesystem staging, and atomic replacement. Remotely updated model metadata is accepted only as bounded canonical JSON signed by an approved ECDSA P-256 key; validity, app/runtime compatibility, artifact fields, monotonic sequence, rollback, and same-sequence equivocation are checked before acceptance. Corrupt durable acceptance state fails closed. The production remote manifest path remains disabled until its public key and fixed endpoint are reviewed. Model and OCR download state use durable generations, so canceled, reconfigured, or stale workers cannot update current state or publish their staging file. Only Settings can enqueue an OCR language pack; connectors and indexing code cannot do so. Current model transport entries remain disabled only because complete reviewed release metadata is absent.
