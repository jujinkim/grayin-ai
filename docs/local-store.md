# Local Store

Grayin AI now uses a SQLCipher-backed local store for derived memory data.

The store is opened through `SqlCipherLocalMemoryStore`. The SQLCipher passphrase is generated locally, encrypted with an Android Keystore AES-GCM key, and kept in app-private preferences. First creation is process-synchronized and committed synchronously so concurrent workers cannot open the same database with different transient passphrases. A partial ciphertext/IV preference pair is treated as corruption and fails closed instead of replacing the database passphrase.

## Accepted Data

`LocalMemoryStore` accepts only:

- `SourceReference`
- `DerivedMemoryEvent`
- `MemoryCitation`
- a reserved `DailyMemorySummary` table/snapshot section only when it is empty in schema v8
- `PlaceCluster`
- a reserved `AppUsageSummary` table/snapshot/scan section only when it is empty in schema v8
- latest derived-only `ConnectorScanStatus`
- connector-scoped delete requests
- index invalidation requests

A connector scan is accepted as one derived-only `ConnectorScanResult`. Source references, events, citations, processing state, bounded scan scope, and typed missing-source issue codes are graph-validated and committed in one transaction. Connector scan status never stores explanation prose. The store rejects blank, duplicate, cross-connector, dangling, code-less, code-mismatched, or unbounded issue lists before any row is written. Schema v8 stores App Usage as canonical per-session derived events and rejects a non-empty `appUsageSummaries` scan output from every connector. Local Files receives an additional closed-schema check: full replacement is mandatory; source/event/citation rows are one-to-one; place and app-usage sections are empty; source kinds are Text, Markdown, or PDF page; source identity is a 64-character HMAC with all pointer/external/app fields absent; events use only canonical structural summaries, bounded keyword signals, exact labels and consistent timestamps/confidence; and citation labels cannot contain a file identity. Local Files has no preferences checkpoint: Sources derives its indexed state from the encrypted graph and latest scan status. A scan with no derived rows still commits its encrypted status before the claimed queue item becomes `NO_INDEXABLE_DATA`, so unsupported and partial-source reasons are not silently lost.

Snapshot-style connectors may set `replaceExistingConnectorData`. The store then removes that connector's old derived graph and inserts the new graph in the same transaction, including defensive cleanup of any legacy aggregate that referenced removed rows. Schema v8 has no canonical daily-summary or app-usage-summary producer, so normal snapshots never contain either. Snapshot replacement is required for a PDF whose page count shrinks, for a user-selected file that disappears, and for every successful Photos MediaStore query so its latest requested range also reconciles deleted or deselected rows. A Photos query that cannot obtain a cursor does not request replacement. Event-driven connectors keep incremental upsert behavior.

Location scans incrementally upsert a stable place cluster for each 0.001-degree rounded coordinate. The same scan transaction unions source references, increments the visit count only for a previously unseen source reference, keeps the earliest and latest observation times, and retains the strongest confidence and widest accuracy radius. Replaying an identical scan is idempotent, and a cluster ID whose centroid changes is rejected rather than silently combining locations.

The store contract must not accept original file bytes, notification originals, message originals, raw calendar records, raw usage logs, raw local-file content, or raw source payloads.

The v2 indexing queue is also SQLCipher-backed. Manual and automatic claims are filtered inside the same transaction that installs the worker lease. Claims exclude any connector with an unexpired running lease, preventing concurrent snapshot replacement for one connector while allowing different connectors to progress. A successful connector execution validates the scan connector ID, claimed item ID, lease owner, attempt number, and unexpired lease in the same SQLCipher transaction that writes derived rows and marks the task complete. A mismatched connector rolls back, and a stale worker returns a lease-lost result before any derived row is written. Other terminal writes require the claimed item ID, lease owner, and attempt number, and expired leases are requeued or failed by a bounded attempt policy.

Schema v3 adds a singleton automatic-control row, queue/runtime generation columns, and generation-scoped automatic-window uniqueness. Disabling automatic indexing performs one transaction that advances control, changes every pending/running automatic task to skipped, clears its lease, and writes the singleton disabled runtime status. Enabled settings changes also advance control and skip older-generation work. Automatic enqueue, claim, derived-row commit, and runtime writes compare against current control in their SQLCipher transaction; manual tasks remain untouched. This both fences in-flight stale output and permits a new generation to enqueue the same window after an off/on cycle. Schema v4 adds one encrypted latest-scan-status row per connector. Schema v5 rewrites legacy scan explanations to fixed issue keys and discards unknown legacy prose as `source_unavailable`. Schema v6 atomically purges legacy Local Files graphs, citations, statuses, and dependent daily summaries before HMAC-only reindexing. Schema v7 adds the connector re-consent barrier used by import. Schema v8 fences pending and running queued work, clears leases, purges every daily-summary and app-usage-summary row because neither has a canonical producer, and removes every legacy Location, Calendar, Photos, Notifications, and App Usage source/event/citation graph plus location clusters and statuses whose older open provider strings could violate the closed derived-record schemas; the next authorized scan rebuilds connector data with canonical values. Live scans and schema-v8 imports then share the same core closed-record validator, which rejects either non-empty reserved aggregate section.

## Read APIs

Read APIs retain all seven wire-compatible sections. `loadSnapshot()` reads source references, events, citations, the reserved empty daily-summary section, place clusters, the reserved empty app-usage-summary section, and latest connector scan status in one database transaction so Ask and encrypted export cannot mix generations. Either non-empty reserved aggregate section is not a canonical schema-v8 snapshot and cannot be imported or exported. Ask adds only scan missing-sources relevant to the planned capabilities and whose stored scan scope contains the planned query range; global statuses remain applicable to unbounded queries.

They do not return raw source content. Query and UI layers must build evidence packs from those derived records only.

## Connector Delete

Connector-level delete first changes that connector's pending and running queue rows to `SOURCE_DATA_DELETED`, clears live leases, then calculates and removes source references, derived memory events, citations, place clusters, legacy app-usage summaries, and latest scan status. The delete path also defensively removes any legacy daily summary that references those records, although the v8 migration purges both aggregate tables and no v8 producer creates either row type. The same transaction fences any in-flight commit before removing data. It also removes connector-prefixed orphan rows left by an interrupted legacy write. Selection and deletion happen in the same transaction; unrelated tasks and connector statuses are preserved.

Index invalidation must run after connector-level delete so retrieval cannot cite stale evidence.

## Security

- SQLCipher encrypts derived memory at rest.
- Android Keystore protects the SQLCipher passphrase.
- The database uses explicit, process-serialized `PRAGMA user_version` migrations: v1 creates derived-memory tables, v2 adds the encrypted indexing queue/runtime tables, v3 adds automatic-control generations, v4 adds connector scan status, v5 converts that status to code-only issue storage, v6 purges legacy Local Files raw identity, v7 adds import re-consent, and v8 removes all legacy daily/app-usage summaries plus legacy open-schema connector records. It rejects schemas newer than this app understands.
- Android backup remains disabled in the manifest, legacy full-backup rules, and Android 12+ cloud/device-transfer extraction rules.
- Explicit export/import uses the separately documented authenticated envelope and the current schema-v8 replace transaction; platform backup and transfer remain disabled.

Schema v7 adds an encrypted connector re-consent barrier. Import clears every derived table plus indexing queue/runtime, fences the automatic generation, inserts the validated seven-section snapshot—with both reserved aggregate sections empty—using conflict-aborting writes, and marks every trusted connector as requiring re-consent in one transaction. Direct and queue-fenced scan writes reject a connector while its barrier exists.
