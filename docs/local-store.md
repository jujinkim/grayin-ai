...
- The database uses explicit, process-serialized `PRAGMA user_version` migrations: v1 creates derived-memory tables, v2 adds the encrypted indexing queue/runtime tables, v3 adds automatic-control generations, v4 adds connector scan status, and v5 converts that status to code-only issue storage. It rejects schemas newer than this app understands.
