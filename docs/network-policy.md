...
All immutable fixed artifacts reject redirects, non-HTTPS endpoints, unexpected content type or encoding, response bodies larger or smaller than the exact catalog byte count, checksum mismatch, and non-atomic publication. Signed manifests use a fixed endpoint and strict maximum body size, then reject non-canonical envelopes/payloads, untrusted keys, invalid signatures, unsafe artifact entries, incompatible/expired metadata, rollback, and equivocation. A failure stores only a stable enum. Existing verified files survive an interrupted replacement.

The current model catalog has no transport entry with a complete immutable URL, exact byte count, and digest. Model download actions are therefore unavailable and `ModelDownloadWorker` fails before opening a connection. The signed-manifest verifier and durable rollback state also remain inactive because no reviewed production P-256 public key or fixed manifest endpoint is configured. Official model pages and local document import remain available. A model network entry may be enabled only after release metadata, production trust configuration, and review are complete.
